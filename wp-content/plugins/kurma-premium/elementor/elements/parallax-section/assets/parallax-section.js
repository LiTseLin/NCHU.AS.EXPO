( function( $, elementor ) {
	...
	window.wpkoiSectionParallax = function( $target ) {
		...
		self.generateEditorSettings = function( sectionId ) {
			...
			if ( ! sectionData.hasOwnProperty( 'wpkoi_parallax_layout_list' ) || 0 === Object.keys( sectionData ).length ) {
				return false;
			}

			sectionParallaxData = sectionData[ 'wpkoi_parallax_layout_list' ].models;
		...
		self.generateLayouts = function() {
			...
			$.each( settings, function( index, layout ) {
				...
				var imageData   = layout['wpkoi_parallax_layout_image'],
					speed       = layout['wpkoi_parallax_layout_speed']['size'] || 50,
